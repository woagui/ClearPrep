      ...
      <div className="container relative z-10 py-20 md:py-32">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Link to="/results">
              <Button size="lg" variant="outline" className="border-border text-foreground hover:bg-muted px-8 py-6 text-base w-full sm:w-auto">
                See Results
              </Button>
            </Link>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            ...
            >
              ...
              <div className="mt-4 inline-block px-3 py-1 rounded-full bg-success/10 text-success text-xs font-semibold">
                +{t.to - t.from} points
              </div>
            </motion.div>
          ))}
          ...
          {comparison.map((row) => (
            <div key={row.feature} className="grid grid-cols-3 text-sm border-b border-border/30 last:border-0">
              ...
              <div className="p-4 flex justify-center">
                {row.us ? <Check className="h-5 w-5 text-success" /> : <X className="h-5 w-5 text-destructive/50" />}
              </div>
              <div className="p-4 flex justify-center">
                {row.them ? <Check className="h-5 w-5 text-success" /> : <X className="h-5 w-5 text-destructive/50" />}
              </div>
            </div>
          ))}
        </motion.div>
